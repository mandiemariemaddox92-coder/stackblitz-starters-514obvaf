...
const DEFAULT_PHOTO = "/placeholder.jpg"

function safeImageSrc(value?: string, fallback = DEFAULT_PHOTO) {
...
export default function ProfilePage() {
  ...
  const creations = useMemo<Creation[]>(() => {
    ...
    const writingCards = userEntries.slice(0, 3).map((entry) => ({
      id: entry.id,
      type: "writing" as const,
      title: entry.content.slice(0, 28) + (entry.content.length > 28 ? "..." : ""),
      caption: `${entry.likes} likes • ${entry.comments} comments`,
    ...
    <div className="mx-auto flex min-h-screen max-w-5xl flex-col px-4 pb-20 pt-14">
      <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-xl">
        <div className="relative h-40 bg-gradient-to-r from-primary/20 via-card to-accent/20">
          {user.coverImage ? (
            <img
              src={safeImageSrc(user.coverImage, DEFAULT_PHOTO)}
              alt="Cover"
          ...
          <div className="-mt-16 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="flex items-end gap-4">
              <div className="h-28 w-28 overflow-hidden rounded-full border-4 border-card bg-secondary shadow-lg">
                <img
                  src={safeImageSrc(user.avatar, DEFAULT_AVATAR)}
                  alt={user.username}
                  className="h-full w-full object-cover"
                />
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                <span className="inline-flex items-center gap-2">
                  ...
                  {isPlaying ? (isMuted ? "Unmute song" : "Mute song") : "Play profile song"}
                </span>
              </button>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-6">
          ...
          {activeTab === "entries" && (
            <div className="space-y-4">
              {userEntries.length === 0 ? (
                <EmptyCard text="No entries yet. Humans love a dramatic blank slate." />
              ) : (
                userEntries.map((entry) => <DiaryEntryCard key={entry.id} entry={entry} />)
              )}
            </div>
          )}
          ...
          {activeTab === "creations" && (
            <div className="space-y-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-accent px-4 py-3 font-semibold text-primary-foreground"
              >
              ...
              {creations.length === 0 ? (
                <EmptyCard text="No creations saved yet." />
              ) : (
                  ...
                  <div key={creation.id} className="flex items-center gap-4 rounded-2xl border border-border bg-card p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-sm text-muted-foreground">
                        {creation.caption || creation.type}
                      </p>
                    </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                    {topFriends.map((friend) => (
                      <FriendCard key={friend.id} friend={friend} onClick={() => setSelectedFriend(friend)} />
                    ))}
                  </div>
                </div>
              )}
          ...
          {activeTab === "private" && (
            <EmptyCard text="Private entries are only visible to you. A rare example of boundaries on the internet." />
          )}
        </div>
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {[...(user.zodiacTraits || []), ...(user.personalityTraits || []), ...(user.interests || [])]
                .slice(0, 10)
                .map((value) => (
      ...
      <audio ref={audioRef} loop>
        <source
          src={user.moodSong?.url || "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"}
        />
      </audio>
      ...
      {showMusicSettings && (
        <MusicSettingsModal
          ...
          onSave={async (song) => {
            await updateCurrentUser({ moodSong: song })
  ...
  return (
    ...
    >
      <div className="mb-3 h-16 w-16 overflow-hidden rounded-full bg-secondary">
        <img
          src={safeImageSrc(friend.avatar, DEFAULT_AVATAR)}
  ...
  return (
    <ModalFrame title="Edit profile" onClose={onClose}>
      <div className="space-y-4">
        ...
        <Field label="Top friend IDs">
          <input
            value={topFriendsInput}
            onChange={(e) => setTopFriendsInput(e.target.value)}
            className="w-full rounded-xl border border-border bg-input px-4 py-3"
            placeholder="2, 3, 4"
          />
        </Field>
        ...
        <Field label="Avatar">
          ...
          />
          {avatar && (
            <div className="mt-3 h-16 w-16 overflow-hidden rounded-full border border-border">
              <img
                src={safeImageSrc(avatar, DEFAULT_AVATAR)}
                alt="Avatar preview"
                className="h-full w-full object-cover"
              />
            </div>
        ...
        <Field label="Cover image">
          ...
          {coverImage && (
            <div className="mt-3 overflow-hidden rounded-2xl border border-border">
              <img
                src={safeImageSrc(coverImage, DEFAULT_PHOTO)}
                alt="Cover preview"
...
function MusicSettingsModal({
  ...
  user: User
  onClose: () => void
  onSave: (song: { title: string; artist: string; url: string }) => Promise<void>
}) {
  ...
  return (
    <ModalFrame title="Upload reel" onClose={onClose}>
      <div className="space-y-4">
        ...
        <button
          disabled={!videoUrl}
          onClick={() => onSave({ title: title || "Untitled Reel", caption, videoUrl })}
  ...
  return (
    <ModalFrame title={friend.displayName} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {(friend.interests || []).slice(0, 4).map((item) => (
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => onTip(5)}
            className="rounded-xl bg-secondary px-4 py-3 text-secondary-foreground"
          >
            Tip $5
          </button>
        </div>
