...
interface ComplimentWheelProps {
  isOpen: boolean
  onClose: () => void
}
...
export function ComplimentWheel({ isOpen, onClose }: ComplimentWheelProps) {
  ...
  const handleSpin = () => {
    ...
    window.setTimeout(() => {
      const random = mockUsers[Math.floor(Math.random() * mockUsers.length)]
      setSelectedUser(random)
      setIsSpinning(false)
    }, 4000)
  }
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div className="relative flex flex-col items-center gap-6 p-8 text-center">
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center animate-float">
            <GiftIcon className="w-10 h-10 text-white" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-foreground mb-2">Kindness Sent</h2>
          ...
          {!selectedUser ? (
            <>
              <div className="text-center mb-8">
                ...
                <p className="text-muted-foreground">
                  Spin to find someone to uplift today
                </p>
              </div>
              ...
              <div className="relative w-64 h-64 mx-auto mb-8">
                ...
                <div 
                  ref={wheelRef}
                  className="w-full h-full rounded-full border-4 border-amber-500/30 overflow-hidden transition-transform duration-[4000ms] ease-out"
                  style={{ 
                    transform: `rotate(${rotation}deg)`,
                    background: 'conic-gradient(from 0deg, #a855f7, #ec4899, #f59e0b, #10b981, #3b82f6, #8b5cf6, #a855f7)'
                  }}
                >
                  {/* User avatars around the wheel */}
                  {mockUsers.slice(0, 6).map((user, i) => {
                    ...
                    return (
                      <div
                        key={user.id}
                        className="absolute w-10 h-10 rounded-full bg-card border-2 border-background flex items-center justify-center text-sm font-bold"
                        style={{
                          left: `${x}%`,
                          top: `${y}%`,
                ...
                <button
                  ...
                  disabled={isSpinning}
                  className={cn(
                    "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center text-white font-bold shadow-lg transition-transform",
                    isSpinning ? "scale-90" : "hover:scale-105 active:scale-95"
                  )}
                >
                  {isSpinning ? (
            ...
            <>
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  className="w-full h-32 p-4 bg-input border border-border rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-amber-500/50 text-foreground placeholder:text-muted-foreground"
                />
              </div>
