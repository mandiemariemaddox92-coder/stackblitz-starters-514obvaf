...
const seedNotifications: NotificationItem[] = [
  { id: "n1", type: "compliment", message: "Velvet Riot spun the wheel and sent you a compliment.", createdAt: new Date(Date.now() - 1000 * 60 * 5), read: false },
  { id: "n2", type: "comment", message: "Ink & Static left a note on your wall.", createdAt: new Date(Date.now() - 1000 * 60 * 35), read: false },
  { id: "n3", type: "follow", message: "Aurora Cut followed your work.", createdAt: new Date(Date.now() - 1000 * 60 * 90), read: true },
]
...
const seedConversations: Conversation[] = [
  {
    ...
    unread: 2,
    messages: [
      { id: "m1", senderId: "2", text: "That playlist idea was ridiculous in the best way.", createdAt: new Date(Date.now() - 1000 * 60 * 120) },
      { id: "m2", senderId: "1", text: "Good. I was aiming for emotionally devastating but useful.", createdAt: new Date(Date.now() - 1000 * 60 * 110) },
    ],
  },
  {
    id: "c2",
    ...
    unread: 0,
    messages: [
      { id: "m3", senderId: "3", text: "Your profile song is haunting.", createdAt: new Date(Date.now() - 1000 * 60 * 300) },
    ],
  },
]
...
interface AppContextType {
  ...
  notifications: NotificationItem[]
  entries: DiaryEntry[]
  addEntry: (entry: Omit<Partial<DiaryEntry>, "id" | "author" | "createdAt" | "likes" | "comments"> & { content: string; font: DiaryFont; privacy: EntryPrivacy; accentColor: string; backgroundColor: string }) => Promise<void>
  conversations: Conversation[]
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const pushNotification = (type: string, message: string) => {
    setNotifications((prev) => [{ id: crypto.randomUUID(), type, message, createdAt: new Date(), read: false }, ...prev])
  }
  ...
  const addProfileComment = (text: string) => {
    const author = currentUser ?? mockCurrentUser
    const newComment: ProfileComment = { id: crypto.randomUUID(), author, text, createdAt: new Date() }
  ...
  const sendMessage = (conversationId: string, text: string) => {
    ...
    setConversations((prev) =>
      prev.map((conversation) => {
        ...
        const message = { id: crypto.randomUUID(), senderId: author.id, text, createdAt: new Date() }
        return {
          ...conversation,
          preview: text,
  ...
  const sendDirectMessage = (userId: string, text: string) => {
    ...
    const newConversation: Conversation = {
      ...
      preview: text,
      unread: 0,
      messages: [{ id: crypto.randomUUID(), senderId: author.id, text, createdAt: new Date() }],
    }
